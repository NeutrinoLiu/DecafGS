...
class Config:
    ...
    # data
    data_dir: str = "examples/scene/coffee"
    data_factor: float = 2
...
def init_gs_and_opt(
    ...
    if init_type == "random":
        ...
        xyz = torch.from_numpy(scene_parser.points).float()
        rgb = torch.from_numpy(scene_parser.points_rgb / 255.0).float()
...
class Runner:
    ...
    def train(self):
        ...
        for step in pbar:
            ...
            gt_batch = data['image'].to(self.device) / 255.0
